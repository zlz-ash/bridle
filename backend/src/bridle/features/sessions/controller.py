...
async def create_session(data: ProjectSessionCreateSchema, db: AsyncSession = Depends(get_db)) -> dict:
...
@router.get("")
async def list_sessions(project_id: str | None = None, db: AsyncSession = Depends(get_db)) -> list[dict]:
...
async def list_messages(session_id: str, db: AsyncSession = Depends(get_db)) -> list[dict]:
...
async def get_capabilities(session_id: str, db: AsyncSession = Depends(get_db)) -> dict:
