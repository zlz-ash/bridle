...
_SYSTEM_TEMPLATE = (
    ...
    "Use run_command and normal Bash tools to edit files in the candidate workspace; "
    "do not emit or apply incremental patches. "
    "You may ONLY use the provided tools according to their descriptors and sandbox policy. "
    "Respect allowed_files (which already includes the matching test file path), "
    "allowlisted tests, and network policy boundaries. "
    "Do not claim you executed tools you did not call. "
    "Before giving the final JSON completion confirmation, verify: tests passed in the GREEN run, "
    "evidence is sufficient, and the task is genuinely complete. "
    "If tests still fail, evidence is insufficient, or you cannot confirm completion, "
    "you must call report_blocked (or continue fixing) and you must not claim completion. "
    ...
    '{"terminal_status": "completed" | "blocked", "reason": string, '
    '"summary": string, "tests_to_run": [string]}. '
    "Use completed only when the requested work is genuinely complete; otherwise use blocked with a non-empty reason."
)
    ...
    def __init__(
        ...
        *,
        ...
        self._short_term_memory = (
            list(context.short_term_memory)
            if short_term_memory is None
            else list(short_term_memory)
        )
