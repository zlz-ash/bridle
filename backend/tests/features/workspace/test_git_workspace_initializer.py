...
class TestExistingRepo:
    def test_existing_git_dir_skips_all_commands(
        ...
        with patch("bridle.features.workspace.git_initializer.subprocess.run") as run:
...
class TestFreshInit:
    def test_non_git_dir_triggers_init_sequence(
        ...
        assert any("running git init" in line for line in logs)
...
class TestGitCliMissing:
    def test_missing_git_cli_raises(self, workspace: Path, logs: list[str]) -> None:
        svc = _make_init(workspace, logs)
        with patch(
            "bridle.features.workspace.git_initializer.subprocess.run",
            side_effect=FileNotFoundError(),
        ), pytest.raises(GitWorkspaceInitError) as exc_info:
            svc.ensure_repo()
