...
from bridle.features.workspace.git_initializer import GitWorkspaceInitError
...
def _serve_context(git_svc: MagicMock | None = None):
    """Patches to short-circuit serve before uvicorn / DB actually run."""
    svc = git_svc if git_svc is not None else MagicMock()
    with (
        patch("bridle.features.workspace.git_initializer.GitWorkspaceInitializer", return_value=svc),
        patch("uvicorn.run"),
