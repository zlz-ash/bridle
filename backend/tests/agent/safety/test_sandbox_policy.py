...
class TestSandboxPolicyPaths:
    ...
    def test_plan_database_patch_is_denied_even_when_allowlisted(self, test_workspace: Path) -> None:
        """Validate a reserved DB path; allowlist input exits denied for generic patch tools."""
        reserved = SandboxPolicy.for_run(
            run_id="r",
            node_id="n",
            workspace_root=test_workspace,
            allowed_files=[".bridle/plan.db"],
            node_tests=[],
        )

        errors = reserved.validate_patch_path(".bridle/plan.db")

        assert any("PlanService" in error for error in errors)
