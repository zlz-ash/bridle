...
class TestWorkspacePathDerivation:
    ...
    def test_aicoding_dir_under_workspace(self, test_workspace) -> None:
        cfg = WorkspaceConfig(test_workspace)
        assert cfg.aicoding_dir == test_workspace / ".aicoding"

    def test_db_path_under_aicoding(self, test_workspace) -> None:
        cfg = WorkspaceConfig(test_workspace)
        assert cfg.db_path == test_workspace / ".aicoding" / "db.sqlite3"
    ...
    def test_runs_dir_under_aicoding(self, test_workspace) -> None:
        cfg = WorkspaceConfig(test_workspace)
        assert cfg.runs_dir == test_workspace / ".aicoding" / "runs"

    def test_logs_dir_under_aicoding(self, test_workspace) -> None:
        cfg = WorkspaceConfig(test_workspace)
        assert cfg.logs_dir == test_workspace / ".aicoding" / "logs"

    def test_reports_dir_under_aicoding(self, test_workspace) -> None:
        cfg = WorkspaceConfig(test_workspace)
        assert cfg.reports_dir == test_workspace / ".aicoding" / "reports"

    def test_context_dir_under_aicoding(self, test_workspace) -> None:
        cfg = WorkspaceConfig(test_workspace)
        assert cfg.context_dir == test_workspace / ".aicoding" / "context"

    def test_current_plan_path_under_aicoding(self, test_workspace) -> None:
        cfg = WorkspaceConfig(test_workspace)
        assert cfg.current_plan_path == test_workspace / ".aicoding" / "current-plan.json"

    def test_plan_summary_path_under_aicoding(self, test_workspace) -> None:
        cfg = WorkspaceConfig(test_workspace)
        assert cfg.plan_summary_path == test_workspace / ".aicoding" / "plan-summary.json"

    def test_aicoding_dirs_are_created_on_access(self, test_workspace) -> None:
        cfg = WorkspaceConfig(test_workspace)
...
class TestPathSafety:
    ...
    def test_all_paths_under_workspace(self, test_workspace) -> None:
        ...
        all_paths = [
            ...
            cfg.reports_dir,
            cfg.context_dir,
            cfg.current_plan_path,
            cfg.plan_summary_path,
        ]
